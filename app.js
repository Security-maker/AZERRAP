...
async function renderQGMissions(){
  ...
  const body = `<section class="grid cols-2">
    <div class="card"><div class="card-title"><div><h2>Créer une mission</h2><p>Planning simple agent / site / horaires</p></div></div>
      <form id="mission-form">
        ...
        <div class="field"><label>Type de mission</label><select class="select" name="type"><option>Surveillance</option><option>Ronde</option><option>Gardiennage</option><option>Événementiel</option><option>Levée de doute</option></select></div>
  ...
  document.querySelector('#mission-form').addEventListener('submit', async e => {
    ...
    const site = sites.find(s=>s.id===fd.get('siteId')); const agent = agents.find(a=>a.id===fd.get('agentId'));
...
function printMissionReport({ mission, shift={}, reports=[] }){
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Rapport mission</title><style>body{font-family:Montserrat,Arial,sans-serif;color:#0b1628;margin:36px}header{display:flex;align-items:center;gap:16px;border-bottom:3px solid #009CFF;padding-bottom:18px;margin-bottom:24px}img{width:70px;height:70px;object-fit:contain}h1{margin:0;font-size:26px}.meta{color:#5E6E82;font-size:13px}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:18px 0}.kpi{border:1px solid #d9e2ec;border-radius:12px;padding:12px}.kpi strong{display:block;font-size:22px}table{width:100%;border-collapse:collapse;margin-top:18px}th,td{border-bottom:1px solid #d9e2ec;text-align:left;padding:10px;vertical-align:top}th{background:#f4f8fb;font-size:11px;text-transform:uppercase}.footer{margin-top:28px;color:#5E6E82;font-size:12px}.signature{margin-top:20px;border:1px solid #d9e2ec;border-radius:12px;padding:14px}@media print{button{display:none}}</style></head><body><header><img src="${logo}"><div><h1>Rapport opérationnel de sécurité</h1><div class="meta">Sentinelle Pro · Export généré le ${new Date().toLocaleString('fr-FR')}</div></div></header><h2>${safe(mission.siteNom || shift.siteNom || 'Site')}</h2><div class="meta">Agent : ${safe(mission.agentNom || shift.agentNom || '—')} · Mission : ${safe(mission.id || '')}<br>Prévu : ${dateText(mission.scheduledStart || shift.scheduledStart)} → ${dateText(mission.scheduledEnd || shift.scheduledEnd)}<br>Réalisé : ${dateText(shift.startTime)} → ${dateText(shift.completedAt)}</div><div class="grid"><div class="kpi"><strong>${reports.length}</strong><span>Rapports</span></div><div class="kpi"><strong>${shift.roundsCount || mission.roundsCount || 0}</strong><span>Rondes</span></div><div class="kpi"><strong>${shift.incidentsCount || mission.incidentsCount || 0}</strong><span>Événements</span></div><div class="kpi"><strong>${shift.conformityScore ?? mission.conformityScore ?? '—'}%</strong><span>Conformité</span></div></div><h3>Main courante de mission</h3><table><thead><tr><th>Heure</th><th>Catégorie</th><th>Gravité</th><th>Message</th></tr></thead><tbody>${rows}</tbody></table><div class="signature"><strong>Signature agent :</strong> ${safe(shift.signatureName || '—')}<br><strong>Note de relève :</strong> ${safe(shift.handoverNote || 'RAS')}</div><div class="footer">Document généré automatiquement. Les horodatages proviennent de Firebase/serveur lorsque disponibles.</div><button onclick="print()">Imprimer / Enregistrer en PDF</button><script>setTimeout(()=>print(),800)<\/script></body></html>`);
  w.document.close();
}

function renderQGReports(){
  currentRoute = 'reports';
  const body = `<section class="card"><div class="card-title"><div><h2>Journal MCI</h2><p>Classement par mission, filtres et traitement des rapports</p></div><button class="btn small" id="export-reports">Export CSV</button></div>
    <div class="form-grid">
  ...
  let rows = [];
  const q = query(collectionRef('reports'), orderBy('createdAt','desc'), limit(300));
  const redraw = () => renderReportsTable(rows);
  unsubscribeList.push(onSnapshot(q, snap => { rows = snap.docs.map(d=>({id:d.id,...d.data()})); refreshReportFilters(rows); redraw(); }));
...
function refreshReportFilters(rows){
  fillSelect('#site-filter', [...new Set(rows.map(r => r.siteNom).filter(Boolean))].sort(), 'Tous les sites');
  fillSelect('#agent-filter', [...new Set(rows.map(r => r.agentNom).filter(Boolean))].sort(), 'Tous les agents');
}
