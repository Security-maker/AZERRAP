...
function renderQGReports(){
  ...
  unsubscribeList.push(onSnapshot(query(collectionRef('sites'), orderBy('name')), snap => {
    qgAllSitesCache = snap.docs.map(d=>({id:d.id,...d.data()}));
    refreshReportFilters(rows);
    redraw();
  }));
  unsubscribeList.push(onSnapshot(query(collectionRef('users'), orderBy('nom')), snap => {
...
');
  ...
  try {
    if (window.showSaveFilePicker) {
      const handle = await window.showSaveFilePicker({ suggestedName:safeFilename, types:[{ description:'CSV', accept:{ 'text/csv':['.csv'] } }] });
    ...
    console.warn('Sauvegarde fichier annulée ou indisponible', error);
  }
  try {
    ...
    console.warn('Partage fichier indisponible', error);
  }
  ...
  try { a.click(); opened = true; } catch(error) { console.warn(error); }
  setTimeout(() => a.remove(), 1500);
  setTimeout(() => URL.revokeObjectURL(url), 120000);
  toast(opened ? 'Export généré. Si le téléchargement ne part pas, utilise le panneau de secours.' : 'Téléchargement bloqué. Utilise le panneau de secours.', opened ? 'success':'warning');
  showDownloadFallback(url, safeFilename, csv);
}
function showDownloadFallback(url, filename, csv){
  const preview = csv.replace(/^﻿/, '').split('
').slice(0, 12).join('
');
  showModal('Téléchargement MCI', `<div class="download-panel"><p class="muted">Sur iPhone/PWA, le téléchargement peut être bloqué par le navigateur. Utilise une des options ci-dessous.</p><div class="grid cols-2"><a class="btn primary full" href="${safe(url)}" download="${safe(filename)}" target="_blank" rel="noopener">Télécharger fichier</a><button class="btn full" id="copy-export-csv">Copier le CSV</button></div><div class="divider"></div><div class="field"><label>Aperçu export</label><textarea class="textarea mono" id="export-preview" readonly rows="10">${safe(preview)}</textarea></div><p class="muted">Méthode sûre sur iPhone : bouton Copier le CSV → ouvrir Notes/Fichiers → coller → enregistrer en .csv.</p></div>`, 'wide');
  document.querySelector('#copy-export-csv')?.addEventListener('click', async () => {
    try {
      await navigator.clipboard.writeText(csv.replace(/^﻿/, ''));
      toast('CSV copié dans le presse-papiers.', 'success');
    } catch(error) {
