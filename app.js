...
function renderIntelResult(result, audience='qg'){
  ...
  return `<div class="intel-result">
    <div class="intel-score-card ${intelLevelClass(result.level)}"><div><div class="muted">Niveau estimé</div><h2>${safe(result.level || 'Normal')}</h2><p>${safe(result.city || '')} · ${safe(result.generatedAtText || nowText())}</p></div><div class="intel-score">${safe(result.score ?? 0)}</div></div>
...
function listenSecurityIntelHistory(selector){
  ...
  unsubscribeList.push(onSnapshot(q, snap => {
    const rows = snap.docs.map(d=>({id:d.id,...d.data()}));
    box.innerHTML = rows.length ? rows.map(r=>`<div class="item compact"><div class="item-main"><div class="item-title">${safe(r.city)} <span class="pill ${intelLevelClass(r.level)}">${safe(r.level)}</span></div><div class="item-meta">${dateText(r.createdAt)} · score ${safe(r.score||0)}/100</div><p>${safe(r.summary||'')}</p></div></div>`).join('') : `<div class="empty">Aucune veille enregistrée.</div>`;
