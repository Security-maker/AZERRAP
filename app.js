...
function exportCSV(rows, filename){
  ...
  try { a.click(); } catch(error) { console.warn(error); }
  setTimeout(() => a.remove(), 2000);
  setTimeout(() => URL.revokeObjectURL(url), 60000);
  toast('Export prêt. Si rien ne se télécharge, utilise le bouton de secours affiché.', 'success');
  showDownloadFallback(url, safeFilename, csv);
}
function showDownloadFallback(url, filename, csv){
  const preview = csv.replace(/^\ufeff/, '').split('\n').slice(0, 8).join('\n');
  showModal('Téléchargement MCI', `<div class="download-panel"><p class="muted">Sur iPhone/PWA, le téléchargement automatique peut être bloqué. Utilise ce bouton puis choisis <strong>Partager</strong> ou <strong>Enregistrer dans Fichiers</strong>.</p><a class="btn primary full" href="${safe(url)}" download="${safe(filename)}" target="_blank" rel="noopener">Télécharger ${safe(filename)}</a><div class="divider"></div><div class="field"><label>Aperçu CSV</label><textarea class="textarea mono" readonly rows="8">${safe(preview)}</textarea></div></div>`);
}
